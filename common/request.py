        ...
        try:
            if interface_url != '':
                ...
                if interface_url.endswith('?'):
                    ...
                    requrl = interface_url +'?'+ temp_interface_param
                print("请求URL地址为：%s" %requrl)
                response = requests.get(url=requrl, params=headerdata,verify=False,timeout=10)
                print("响应结果为： %s" %(response.json()))
                #print response
                if response.status_code==200:
                    durtime = (response.elapsed.microseconds) / 1000  # 发起请求和响应到达的时间,单位毫秒
...
if __name__ == "__main__":
    ...
    sen_sql= "select exe_mode,url_interface,header_interface,params_interface from case_interface where name_interface='open_app' and  id=3"
    ...
    if params_interface['code']=='0000':
        ...
        if url_interface !='' and headerdata != '' and param_interface !='' and type_interface != '':
            ...
            if result['code']=='0000':
                ...
                test_db.op_sql("UPDATE case_interface SET result_interface='%s' WHERE id=3" %result_resp)#将结果更新到case_interface表中
